...
class SalidasEdit extends Component
{
    use LivewireAlert;
    use CalcularSalida;
    use DataSalida;
    ...
    {
        $this->movimiento = MovimientoAlmacen::with('movimientoDetalles')->find($salida);
    ...
    {
        $this->update();
        return view('livewire.salidas.salidas-edit')->extends('layouts.tema.app')->section('content');
    }
    ...
    {
        $this->motivos = Motivo::where('tipo', 'S')->get();
    }
    ...
    public function actualizarSalida()
    {
        ...
        return redirect()->route('salida.show', $this->movimiento->id);
    }
}
